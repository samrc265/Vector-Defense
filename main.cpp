...
bool DrawCustomButton(Rectangle bounds, const char* text, Color baseCol, int fontSize = 24) {
    ...
    if (hovering && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) PlaySound(sndBlip);
...
int main() {
    ...
    float empTimer = 0.0f, overdriveTimer = 0.0f, empWaveRadius = 0.0f, pulseVisualRadius = 0.0f, shakeIntensity = 0.0f, damageFlashTimer = 0.0f;
    ...
    while (!WindowShouldClose()) {
        ...
        switch (currentScreen) {
            ...
            case UPGRADE_MENU: if (IsKeyPressed(KEY_U) || IsKeyPressed(KEY_ENTER)) {
                    currentScreen = GAMEPLAY;
                    if (pendingCryoNotify) { notifications.push_back({"CRYO-TECH UNLOCKED!", 5.0f, V_SKYBLUE}); notifications.push_back({"PRESS [2] TO SELECT", 5.0f, V_WHITE}); pendingCryoNotify = false; }
                    if (pendingTeslaNotify) { notifications.push_back({"TESLA-TECH UNLOCKED!", 5.0f, V_GOLD}); notifications.push_back({"PRESS [3] TO SELECT", 5.0f, V_WHITE}); pendingTeslaNotify = false; }
                }
                break;
            ...
            case GAMEPLAY: {
                ...
                if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) && !mouseOnUi) {
                    ...
                    for(auto &p : powerups) {
                        if(p.active && GetDistance(mousePos, p.position) < 45) {
                            ...
                            else if(p.type == PWR_HEAL) { coreHealth = std::min(coreHealth + 3, maxCoreHealth); notifications.push_back({"INTEGRITY RESTORED", 2.0f, V_CYAN}); 
                ...
                for (auto &t : towers) {
                    ...
                    if (t.shootTimer >= rate) {
                        ...
                        if (target) {
                            PlaySound(sndShoot); shakeIntensity += 1.5f; // Tiny kickback shake
        ...
        BeginDrawing();
            ...
            if (currentScreen == START_MENU) {
                if (DrawCustomButton({ SCREEN_WIDTH/2 - 150, 360, 300, 65 }, "BOOT SEQUENCE", V_LIME)) { PlaySound(sndBlip); currentScreen = GAMEPLAY; waveActive = false; }
                if (DrawCustomButton({ SCREEN_WIDTH/2 - 150, 440, 300, 65 }, "LEADERBOARD", V_GOLD)) { PlaySound(sndBlip); currentScreen = LEADERBOARD; }
                if (DrawCustomButton({ SCREEN_WIDTH/2 - 150, 520, 300, 65 }, "SYSTEM GUIDE", V_WHITE)) { PlaySound(sndBlip); currentScreen = GUIDE; }
                ...
                if (DrawCustomButton({ SCREEN_WIDTH/2-100, 620, 200, 50 }, "< RETURN", V_WHITE)) { PlaySound(sndBlip); currentScreen = START_MENU; }
                ...
                if (waveActive) { if (pulseWaveCharges > 0) DrawCustomButton(pulseRect, TextFormat("ACTIVATE PULSE [%d]", pulseWaveCharges), V_SKYBLUE, 18); DrawText(TextFormat("THREATS: %d", (int)enemies.size() + enemiesToSpawn + (bossInQueue?1:0)), 25, SCREEN_HEIGHT - 35, 20, V_SKYBLUE); }
                ...
                if (currentScreen == GAMEPLAY && !waveActive && enemies.empty()) {
                    ...
                    if (DrawCustomButton({ SCREEN_WIDTH - 550, SCREEN_HEIGHT - 72, 250, 60 }, "OPEN ARMORY [U]", V_GOLD)) { PlaySound(sndBlip); currentScreen = UPGRADE_MENU; }
                    if (DrawCustomButton({ SCREEN_WIDTH - 280, SCREEN_HEIGHT - 72, 250, 60 }, "START WAVE", V_LIME)) { PlaySound(sndBlip); currentWave++; waveActive = true; enemiesToSpawn = 7 + (currentWave * 5); if (currentWave % 10 == 0) bossInQueue = true; }
                }
                if (currentScreen == UPGRADE_MENU) {
                    ...
                    if (DrawCustomButton({ SCREEN_WIDTH/2 - 200, 180, 400, 65 }, TextFormat("BUY NODE SLOT (%d)", sC), V_LIME)) { if(currency >= sC){ PlaySound(sndBlip); currency -= sC; maxTowers++; if(maxTowers==5&&!cryoUnlocked){ cryoUnlocked=true; pendingCryoNotify=true;} if(maxTowers==7&&!teslaUnlocked){ teslaUnlocked=true; pendingTeslaNotify=true;} } }
                    if (DrawCustomButton({ SCREEN_WIDTH/2 - 200, 260, 400, 65 }, "PULSE CHARGE (300)", V_SKYBLUE)) { if(currency >= 300){ PlaySound(sndBlip); currency -= 300; pulseWaveCharges++; } }
                    if (DrawCustomButton({ SCREEN_WIDTH/2 - 200, 340, 400, 65 }, TextFormat("OVERCLOCK FIRE (%d)", fC), V_GOLD)) { if(currency >= fC){ PlaySound(sndBlip); currency -= fC; towerFireRate *= 0.85f; } }
                    if (DrawCustomButton({ SCREEN_WIDTH/2 - 200, 420, 400, 65 }, "CORE REPAIR (450)", V_CYAN)) { if(currency >= 450 && coreHealth < maxCoreHealth){ PlaySound(sndBlip); currency -= 450; coreHealth = std::min(coreHealth+6, maxCoreHealth); } }
                ...
                DrawText("MISSION PERFORMANCE LOG", bX + 40, bY + 30, 26, V_SKYBLUE); DrawText(TextFormat("TOTAL HARVESTED DATA: %d", score), bX + 40, bY + 90, 20, V_WHITE); DrawText(TextFormat("FINAL WAVE DEPTH: %d", currentWave), bX + 40, bY + 125, 20, V_WHITE); DrawText(TextFormat("REMAINING FRAGMENTS: %d", currency), bX + 40, bY + 160, 20, V_GOLD);
                DrawText("FINAL CONFIGURATION:", bX + 440, bY + 90, 20, V_LIME); DrawText(TextFormat("- ACTIVE NODE SLOTS: %d", maxTowers), bX + 440, bY + 125, 18, V_WHITE); DrawText(TextFormat("- LASER RECHARGE: %.2fs", towerFireRate), bX + 440, bY + 155, 18, V_WHITE);
                if (!scoreSaved) { DrawText("RECOVER SURVIVOR DATA?", bX + 40, bY + 230, 22, V_CYAN); DrawRectangle(bX + 40, bY + 270, 300, 50, ColorAlpha(V_DARKGRAY, 0.5f)); DrawRectangleLines(bX + 40, bY + 270, 300, 50, V_CYAN); DrawText(playerName, bX + 55, bY + 282, 24, V_WHITE); if ((GetTime() * 2) - (int)(GetTime() * 2) > 0.5) DrawRectangle(bX + 55 + MeasureText(playerName, 24), bY + 280, 15, 30, V_WHITE); if (DrawCustomButton({ (float)bX + 360, (float)bY + 270, 200, 50 }, "SAVE DATA", V_CYAN, 20)) { SaveScore(playerName, score); scoreSaved = true; } } else { DrawText("SURVIVOR DATA SYNCED TO HALL OF FAME", bX + 40, bY + 282, 22, V_LIME); }
                if (DrawCustomButton({ SCREEN_WIDTH/2 - 150, 600, 300, 65 }, "REBOOT SYSTEM", V_GOLD)) { PlaySound(sndBlip); score = 0; currency = 0; currentWave = 0; pulseWaveCharges = 0; coreHealth = maxCoreHealth; towers.clear(); enemies.clear(); lasers.clear(); powerups.clear(); particles.clear(); notifications.clear(); maxTowers = 3; towerFireRate = 0.8f; currentScreen = GAMEPLAY; waveActive = false; cryoUnlocked = false; teslaUnlocked = false; pendingCryoNotify = false; pendingTeslaNotify = false; }
            }
        EndDrawing();
    }
    UnloadSound(sndBlip); UnloadSound(sndBoom); UnloadSound(sndShoot); UnloadShader(bloom); UnloadRenderTexture(target); CloseAudioDevice(); CloseWindow(); return 0;
}